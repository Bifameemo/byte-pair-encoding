...
def construct_table(text: str) -> list[Token]:
    ...
    while True:
        ...
        # build new_tokenised_text and update pair_frequencies
        new_tokenised_text: list[Token] = []
        i: int = 0
        while i <= len(tokenised_text) - 2:
            ...
            if pair == max_pair:
                ...
                # replace second half of pair with max_pair
                # so that subsequent "before_pair"s see max_pair
                tokenised_text[i + 1] = max_pair
                # append max_pair instead of pair to new_tokenised_text to replace pair
                new_tokenised_text.append(max_pair)
                i += 2
            else:
                new_tokenised_text.append(pair)
                i += 1
        tokenised_text = new_tokenised_text
...
with open(r"datasets\frankenstein-letters.txt", 'r') as file:
    text: str = file.readline()
print("input text contains", len(text), "characters")

import time
start = time.time()
table: list[Token] = construct_table(text)
end = time.time()
print("Constructed table in", end - start, "seconds")
